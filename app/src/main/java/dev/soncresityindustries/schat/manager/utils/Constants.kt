package dev.soncresityindustries.schat.manager.utils

import android.os.Environment
import dev.soncresityindustries.schat.manager.BuildConfig

object Constants {


    val TEAM_MEMBERS = listOf(
        TeamMember("Kalarian Athecila", "CEO of Soncresity Industries", "kalarianathecila"),
        TeamMember("SkyKing_PX", "Lead Developer at Soncresity Industries", "SkyKingPX")
    )
    val BUNNY_DEVS = listOf(
        TeamMember("Pylix", "Developer - Bunny & Vendetta", "pylixonly"),
        TeamMember("Kasi", "Developer - Xposed Module", "redstonekasi")
    )

    val VENDETTA_DEVS = listOf(
        TeamMember("Fiery", "Lead Dev - iOS", "FieryFlames"),
        TeamMember("Maisy", "Creator - Vendetta", "maisymoe"),
        TeamMember("Wing", "Lead Dev - Manager", "wingio")
    )

    // NOTE: This is no longer used
    val VENDETTA_DIR = Environment.getExternalStorageDirectory().resolve("SChat")

    val DUMMY_VERSION = DiscordVersion(1, 0, DiscordVersion.Type.STABLE)

    val START_TIME = System.currentTimeMillis()
}

object Intents {

    object Actions {
        const val INSTALL = "${BuildConfig.APPLICATION_ID}.intents.actions.INSTALL"
    }

    object Extras {
        const val VERSION = "${BuildConfig.APPLICATION_ID}.intents.extras.VERSION"
    }

}

object Channels {
    const val UPDATE = "${BuildConfig.APPLICATION_ID}.notifications.UPDATE"
}

data class TeamMember(
    val name: String,
    val role: String,
    val username: String = name
)