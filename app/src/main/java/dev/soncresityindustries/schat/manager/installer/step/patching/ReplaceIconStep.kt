...
import dev.soncresityindustries.schat.manager.installer.util.ArscUtil
import dev.soncresityindustries.schat.manager.installer.util.ArscUtil.addColorResource
import dev.soncresityindustries.schat.manager.installer.util.ArscUtil.getMainArscChunk
import dev.soncresityindustries.schat.manager.installer.util.ArscUtil.getPackageChunk
import dev.soncresityindustries.schat.manager.installer.util.ArscUtil.getResourceFileName
import dev.soncresityindustries.schat.manager.installer.util.AxmlUtil
...
class ReplaceIconStep : Step() {
    ...
    override suspend fun run(runner: StepRunner) {
        ...
        val squareIconFile =
            arsc.getMainArscChunk().getResourceFileName(iconRscIds.squareIcon, "anydpi-v26")
        val roundIconFile =
            arsc.getMainArscChunk().getResourceFileName(iconRscIds.roundIcon, "anydpi-v26")

        runner.logger.i("Patching icon assets (squareIcon=$squareIconFile, roundIcon=$roundIconFile)")

        val backgroundColor =
            arsc.getPackageChunk().addColorResource("schat_color", Color(0xFF48488B))
        ...
        for (rscFile in setOf(
            squareIconFile,
            roundIconFile
        )) { // setOf to not possibly patch same file twice
